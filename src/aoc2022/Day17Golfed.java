...
c=c.ZERO;h=0;var J=j[0];var m=(new java.util.HashMap());long n=p?2022:0xe8d4a51000L,r=0,e=0;int q=0;
for(;r<n;){
k=(Math.floorMod(r,5));int x=2,y=h+3,X;
for(;;){
X=x+J.charAt(q)-61;q=++q%J.length();
if(o(X,y))x=X;
if(o(x,y-1))y--;else{c=c.or(c.valueOf(P[k]).shiftLeft(x+7*y));h=(Math.max(h,y+H[k]));break;}
}
r++;if(e<1){
int f=h,i=7,z;
for(;i-->0;f=(Math.min(f,z<0?f:z+1)))for(z=h;z-->0&&!c.testBit(7*z+i););
