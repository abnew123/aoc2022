    ...
    private static Verification verify() throws Exception {
        ...
        for (int day = 1; day <= DAY_COUNT; day++) {
            ...
            for (int part = 0; part < 2; part++) {
                ...
                String independentAnswer = independentAnswers.get(answerIndex(day, part));
                if (!answers[part].equals(independentAnswer)) {
                    throw new IllegalStateException(String.format(Locale.ROOT,
                            "Day %02d part %d fullSolve mismatch: solve <%s>, fullSolve <%s>",
                            day, part + 1, independentAnswer, answers[part]));
                }
    ...
    private static void runChild(long mainStartNanos) throws Exception {
        ...
        for (int day = 1; day <= DAY_COUNT; day++) {
            ...
            if (dayAnswers == null || dayAnswers.length != 2
                    || dayAnswers[0] == null || dayAnswers[1] == null) {
                throw new IllegalStateException(String.format(Locale.ROOT,
                        "Day %02d fullSolve did not return two non-null answers", day));
            }
            answers.add(dayAnswers[0]);
            answers.add(dayAnswers[1]);
        }

        String answerChecksum = checksum(answers);
        long mainNanos = System.nanoTime() - mainStartNanos;
        System.out.printf(Locale.ROOT,
                "%sprotocol=%s main_ns=%d solver_ns=%d checksum=%s%n",
                RESULT_PREFIX, PROTOCOL_VERSION, mainNanos, solverNanos, answerChecksum);
    }
    ...
    private static ChildResult parseChildResult(String line) {
        ...
        for (String field : fields) {
            ...
            switch (pair[0]) {
                ...
                case "checksum" -> {
                    requireMissing(pair[0], answerChecksum);
                    answerChecksum = pair[1];
                }
        ...
        if (!PROTOCOL_VERSION.equals(protocol)
                ...
                || answerChecksum == null || !answerChecksum.matches("[0-9a-f]{64}")) {
            throw new IllegalStateException("Incomplete child result: " + line);
        }
        return new ChildResult(mainNanos, solverNanos, answerChecksum);
    }
